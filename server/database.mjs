...
export function normalizeDatabaseUrl(value = process.env.DATABASE_URL) {
  if (!value?.trim()) {
    throw databaseConfigurationError('DATABASE_URL is not configured.');
  }
  ...
  // Recover the common deployment mistake shown by Neon/Vercel when only the
  // credential and host portion is pasted and the postgres scheme is omitted.
  if (!/^[a-z][a-z\d+.-]*:\/\//i.test(candidate)) {
    if (/^[^\s/:?#]+:[^\s@]+@[^\s/]+\/[^\s/?#]+/i.test(candidate)) {
      candidate = `postgresql://${candidate}`;
    } else {
      throw databaseConfigurationError(
        'DATABASE_URL must be a complete PostgreSQL connection string.',
      );
    }
  }
  ...
  if (
    ...
    throw databaseConfigurationError(
      'DATABASE_URL must include the PostgreSQL scheme, credentials, host, and database name.',
    );
  }
...
export function getSql() {
  ...
  try {
    ...
    throw databaseConfigurationError(
      'DATABASE_URL is invalid. Copy the pooled connection string from Neon and redeploy.',
    );
  }
  return queryClient;
}
